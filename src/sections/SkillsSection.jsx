        ...
        <div className="shape-skills grid gap-4 lg:grid-cols-5">
          {Object.entries(skills).map(([category, items], index) => {
            ...
            return (
              ...
              >
                <h3 className="inline-flex items-center gap-2 text-lg font-bold text-[var(--text)]">
                  <CategoryIcon className="text-[var(--brand)]" />
                  {category}
                </h3>

                <div className="mt-4 flex flex-wrap gap-2">
                  {items.map((item) => {
                    const ItemIcon = skillItemIconMap[item] || FiCode

                    return (
                      <motion.span
                        key={item}
                        className="pill skill-pill inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)]"
                        whileHover={{ y: -2, scale: 1.02 }}
                        transition={{ duration: 0.18 }}
                      >
                        <ItemIcon className="text-[var(--brand)]" />
                        {item}
                      </motion.span>
                    )
                  })}
